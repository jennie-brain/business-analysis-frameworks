...
| SOM | USD-KRW 코리도·중소중견기업 대상, 5년차 기준 확보 목표 | **약 62억~94억 달러**(2~3% 가정) | 7단계 |
...
|---|---|---|
| Phase 1(1년차) | 파일럿 단계, 상용 점유율 0% | 파일럿 거래 한정 |
| Phase 2(3년차) | 1% | 약 31억 달러 |
| Phase 3(5년차) | 2~3% | 약 62억~94억 달러 |

**SOM(5년차 기준 시나리오) ≈ 62억~94억 달러**

이 점유율(1~3%)은 데이터로 확정된 값이 아니라, 은행 파트너십이 필요한 도매 인프라 기반 B2B 핀테크가 통상 3~5년 내 확보하는 수준을 참고한 가정치다.
...
flowchart TD
    ...
    C --> D["SOM(5년차 시나리오)<br/>약 62억~94억 달러"]
```

**그림 1. TAM·SAM·SOM 퍼널** — TAM에서 SAM으로 좁힐 때는 지역(한국), SAM에서 타겟 세그먼트로 좁힐 때는 통화(USD)와 고객 규모(중소·중견), 타겟 세그먼트에서 SOM으로 좁힐 때는 확보 가능한 점유율(1~3%)이 각각의 축이다.
