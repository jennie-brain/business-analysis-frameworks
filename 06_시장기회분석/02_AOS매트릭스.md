...
X축은 Satisfaction, Y축은 Importance다. 두 축 모두 1~5점을 0~1로 선형 변환했다(0=1점, 1=5점). Importance=3점(11건)은 중앙값과 정확히 같아 분할선(0.5) 위에 찍힌다 — "이상=High" 규칙으로 Q1/Q2 쪽에 포함시켰다. 다수 항목이 Importance·Satisfaction 조합이 동일해 좌표가 겹친다(예: 한지원⑤·오세훈⑤·이수경⑤은 모두 (0.5, 0.25)).

```mermaid
quadrantChart
    title AOS 매트릭스 36개 Pain (X축 Satisfaction, Y축 Importance)
...
**그림. AOS 매트릭스(36개)** — Q1(좌상단)에 16개가 몰려 있고, 그중 강태민②·배소연②(0.0, 1.0)가 가장 왼쪽 위 극단에 위치한다. Q3(좌하단)에는 이영재⑤(0.25, 0.25)·오현석⑤(0.25, 0.25)만 있다.
