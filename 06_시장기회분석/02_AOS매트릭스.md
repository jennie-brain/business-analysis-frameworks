...
사분면을 가르는 기준은 Importance·Satisfaction 각각 **중앙값 3점**이다 — 두 축 모두 3점 이상을 High, 1~2점을 Low로 분류한다. 이 기준으로는 축 안에서 정확히 몇 점인지(1점인지 2점인지, 4점인지 5점인지)가 사분면 소속을 바꾸지 않으므로, 매트릭스는 1~5점 전체를 좌표로 찍는 대신 **Low/High 2단계 × 2축 = 4개 사분면**에 36개를 그대로 나눠 담았다. mermaid `quadrantChart`는 GitHub 렌더러에서 라벨 문자와 무관하게 반복적으로 렉싱 오류를 내, 05챕터 CJM 인포그래픽과 같은 방식(직접 그린 SVG)으로 교체했다. 항목 라벨은 5절 표의 "#" 컬럼과 동일한 ID이고, 뒤의 숫자는 AOS 값이며 ★는 05챕터 대표 4명(C1·A2·E1·N1)이다.

![AOS 매트릭스](../이미지/AOS_매트릭스.svg)

**그림. AOS 매트릭스(36개, 4분면 전체 나열)** — 각 사분면 안에서 AOS 내림차순으로 정렬했다. Q1은 16건으로 가장 크고 최고점(4.0)은 강태민②·배소연②(E1-2★·E2-2)다. Q3(좌하단)에는 이영재⑤·오현석⑤(A1-5·N2-5) 2건만 있다. Importance·Satisfaction 정확한 점수는 5절 표를 참고한다.
