...
X축은 Satisfaction, Y축은 Importance이고, 두 축 모두 **3점에 굵은 분할선**을 그려 4개 사분면을 만들었다(3점 이상 = High). 36개 Pain은 두 점수가 모두 정수(1~5)라 같은 좌표에 여러 건이 겹치는데, 굳이 인위적으로 흩뜨리지 않고 **겹치는 항목을 하나의 점으로 묶어 점 안에 건수를 표시**하고, 옆에 해당 항목들을 나열했다. mermaid `quadrantChart`는 GitHub 렌더러에서 라벨 문자와 무관하게 반복적으로 렉싱 오류를 내, 05챕터 CJM 인포그래픽과 같은 방식(직접 그린 SVG)으로 교체했다. 항목 라벨은 5절 표의 "#" 컬럼과 동일한 ID이고 ★는 05챕터 대표 4명(C1·A2·E1·N1)이다. 3점(경계값)을 받은 항목은 분류상 High이므로, 점 위치를 경계선에서 자기 사분면 쪽으로 살짝 옮겨 표시했다.

![AOS 매트릭스](../이미지/AOS_매트릭스.svg)

**그림. AOS 매트릭스(36개, 실제 좌표)** — 11개의 점에 36개가 나뉘어 찍혀 있다. 가장 큰 점(Sat2, Imp4)은 7건(C2-1·C4-1·C5-1·A2-2★·A1-2·A3-2·N2-3)이 겹친 자리다. 강태민②·배소연②(E1-2★·E2-2)는 (Sat1, Imp5)에서 Q1 중에서도 가장 왼쪽 위 극단에 위치한다. Q3(좌하단)에는 이영재⑤·오현석⑤(A1-5·N2-5) 2건만 있다. Importance·Satisfaction 정확한 점수는 5절 표를 참고한다.
