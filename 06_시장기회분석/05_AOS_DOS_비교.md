...
이 문서는 [00_방법론.md](00_방법론.md) 5절(AOS·DOS가 충돌할 때의 판단 기준)을 36개 Pain 전체에 실제로 적용한 결과다. AOS([02_AOS매트릭스.md](02_AOS매트릭스.md))와 DOS·SOM 버전([04_DOS_SAM-SOM버전.md](04_DOS_SAM-SOM버전.md))을 나란히 놓고, 두 지표가 각각 High/Low일 때의 조합으로 4개 케이스에 배치했다.

- **기준선**: AOS는 36개 중앙값인 **1.2**를 기준으로 High(≥1.2)/Low(<1.2)를 가른다. DOS는 **0을 기준**으로 High(양수)/Low(0 이하)를 가른다 — [02_AOS매트릭스.md](02_AOS매트릭스.md)에서 이미 확인했듯 36개 DOS(SOM 버전)의 중앙값 자체가 정확히 0이라, "양수=시장가중 기회 있음"이라는 직관적 해석과 통계적 중앙값이 일치한다.
- **4개 케이스**([00_방법론.md](00_방법론.md) 5절 원문 기준): ① BEST CASE(AOS·DOS 둘 다 높음) ② AOS 높음·DOS 낮음("고객은 고통스럽지만 시장은 작다") ③ DOS 높음·AOS 낮음("시장은 크지만 고객은 덜 고통스럽다") ④ 둘 다 낮음(원문에 없는 케이스지만 저우선순위로 분류).
...
- BEST CASE 16개 중 7개(A2-②④, A1-②④, A3-②④⑤)가 Adjacent(정민재·이영재·한도윤) 소속이다 — Adjacent 9개 Pain 중 A2-①·A1-⑤ 2개는 "AOS 높음·DOS 낮음" 그룹에 속해 BEST CASE에서 제외된다 — AOS·DOS 판정만 보면 이들도 "최우선"이지만, [03_MarketRelevance_판단.md](03_MarketRelevance_판단.md)가 이미 지적했듯 Market Relevance 자체가 잠정치(0.2)라 이 판정은 **시장 규모를 실측하기 전까지는 잠정 BEST**로 읽어야 한다.

> ⚠️ **유의점 — BEST CASE 공동 1위(E1-②·E2-②)는 미검증 가설에 근거한다.** 두 항목의 핵심 근거인 "동시 마감 시 컷오프가 밀려 실제로 위약금을 냈다"는 사건은 05챕터 01문서 4단계가 이미 "원자료에 직접 인용이 없는 가설"로 표시해 둔 것이다. 은행 컷오프 시간·분기말 유동성 압박·지연배상금 조항이라는 개별 메커니즘 자체는 업계에서 잘 문서화된 실재하는 관행이지만([01_Pain리스트_Goal추론.md](01_Pain리스트_Goal추론.md) "외부 근거 보충" 절 참고), 이 셋이 실제로 결합해 "SME 수출입기업이 위약금을 낸" 사례를 뒷받침하는 업계 통계·사례는 확인되지 않았다. 즉 06챕터 전체(AOS→DOS→AOS×DOS)에서 일관되게 1위로 나온 항목이 **아직 실사되지 않은 추론**이라는 뜻이며, 07챕터 JTBD 인터뷰에서 발생 빈도·규모를 최우선으로 검증해야 한다.
...
```mermaid
quadrantChart
    title AOS vs DOS(SOM) mixed matrix, 20 clusters
...
### 1.2 SAM 버전 — Mermaid 혼합 매트릭스

같은 방식을 [04_DOS_SAM-SOM버전.md](04_DOS_SAM-SOM버전.md) 3절의 DOS(SAM 버전) 값에 그대로 적용했다. AOS는 Market Relevance와 무관하게 Importance·Satisfaction만으로 정해지므로 X축 좌표는 SOM 버전과 완전히 동일하다 — 즉 두 차트는 같은 AOS 열에서 Y축(DOS)만 다르게 움직이므로 나란히 비교하기에 알맞다. Y축 정규화도 SOM 버전과 같은 방식(0을 0.5로 고정하고, 최댓값 0.92를 0.99에, 최솟값 -1.16을 0.0에 선형으로 맞춤)을 썼다.

**왜 20개가 아니라 18개인가.** SAM 버전에서는 Market Relevance 5개 그룹 값이 **수출 0.15·Adjacent 0.15(잠정)로 우연히 같아진다.** 그 결과 (Imp−Sat) 격차가 같은 수출 그룹과 Adjacent 그룹 항목은 DOS 좌표까지 겹친다 — 이수경(C2-①, 수출, 격차2)이 정민재★·이영재·한도윤 5건(Adjacent, 전부 격차2)과 합쳐져 6건짜리 MIX_6이 되고, 배소연(E2-③, 수출, 격차1)이 정민재★·한도윤 2건(Adjacent, 격차1)과 합쳐져 3건짜리 ADJ_3이 된다. 반대로 한지원(C4-①, 수입, Relevance 0.08)은 어느 그룹과도 값이 겹치지 않아 SOM 버전과 마찬가지로 단독 좌표를 유지한다. 이 병합은 **시장을 실측해서 나온 결과가 아니라는 점을 주의해야 한다** — 수출 0.15는 04챕터 실측 비중(2,013억÷1.34조)이지만 Adjacent 0.15는 "SOM판(0.2)보다 다소 낮춰 잡은" 잠정 추정치이므로, 두 그룹이 정확히 같은 자리에 겹치는 것은 우연에 가깝다. 나머지 클러스터(겸업 3건군 RESID_3, 겸업 2건군 BEST_2, 수출 2건군 LOCKIN_2·RESID_2, NEUTRAL_7 등)는 같은 그룹 내부 병합이라 SOM 버전과 멤버 구성이 동일하고 Y좌표만 달라진다.

| 라벨 | 항목(#) | AOS | DOS(SAM) |
|---|---|---|---|
| C1_1 | C1-① 김민준★ | 3.0 | 0.45 |
| C3_1 | C3-① 정하늘 | 3.0 | 0.24 |
| NEUTRAL_7 | Imp=Sat 7건(C1-⑤·A2-①·C3-⑤·C4-④·C5-④·A1-⑤·N2-⑤) | 1.2 | 0.0 |
| LOCKIN_2 | C1-⑥·C2-④ | 0.4 | -0.3 |
| RESID_2 | C2-⑤·E2-⑤ | 0.8 | -0.15 |
| C3_6 | C3-⑥ 정하늘 | 0.4 | -0.16 |
| C4_1 | C4-① 한지원 | 2.4 | 0.16 |
| C4_5 | C4-⑤ 한지원 | 0.8 | -0.08 |
| BEST_2 | C5-①·N2-③ | 2.4 | 0.46 |
| RESID_3 | C5-⑤·E1-⑥·N2-① | 0.8 | -0.23 |
| MIX_6 | C2-①·A2-②·A1-②·A1-④·A3-②·A3-⑤ | 2.4 | 0.3 |
| ADJ_3 | A2-④·A3-④·E2-③ | 1.8 | 0.15 |
| E1_2 | E1-② 강태민★(최고점) | 4.0 | 0.92 |
| E1_5 | E1-⑤ 강태민★ | 0.4 | -0.46 |
| E2_2 | E2-② 배소연 | 4.0 | 0.6 |
| N1_1 | N1-① 최지훈★ | 0.0 | -1.16 |
| N1_3 | N1-③ 최지훈★ | 0.4 | -0.58 |
| N1_4 | N1-④ 최지훈★ | 0.0 | -0.58 |

```mermaid
quadrantChart
    title AOS vs DOS(SAM) mixed matrix, 18 clusters
    x-axis Low AOS --> High AOS
    y-axis Low DOS --> High DOS
    quadrant-1 Best case
    quadrant-2 Low AOS high DOS
...
사분면 소속(BEST CASE·AOS高DOS低·DOS高AOS低·둘다낮음)은 **SOM·SAM 어느 버전을 쓰든 바뀌지 않는다** — Market Relevance는 항상 양수라 DOS의 부호(=Imp−Sat의 부호)를 바꾸지 못하기 때문이다. 즉 2~5절의 16/7/0/13건 분류는 SAM 버전으로 다시 그려도 그대로 유지되며, 달라지는 것은 오직 **BEST CASE 내부의 상대적 위치**뿐이다. SOM 버전 차트에서는 Adjacent(ADJ_5)가 수출·겸업 그룹보다 왼쪽 아래(낮은 DOS)에 처져 있었지만, SAM 버전 차트에서는 수출 그룹(이수경·배소연)과 같은 좌표로 올라와 사실상 구분이 안 될 정도로 붙는다 — [04_DOS_SAM-SOM버전.md](04_DOS_SAM-SOM버전.md)가 표로 확인한 "Adjacent는 SAM 관점에서 저평가가 풀린다"는 결론을 산점도로도 그대로 재확인한 셈이다.
...
[00_방법론.md](00_방법론.md) 5절 원문의 전략 방향(**"반복적으로 고객을 확보·유지하는 데 용이한 비즈니스"**)을 그대로 적용하면, 이 7개는 시장 규모를 키우기보다 **개별 고객 리텐션·반복 사용 유도**로 접근해야 한다는 뜻이다.
...
이 13개 중 6개(C2-⑤·C4-⑤·C5-⑤·E1-⑥·E2-⑤·N2-①)는 **도입 후 잔존 마찰**(강태민·오세훈의 위험예측 기능 도입 후 남은 승인 부담 등)이라 이미 어느 정도 관리되고 있는 항목이고, 나머지는 최지훈(대기업, 3개) + 락인 리스크류다. 최지훈의 3개 Pain 전부가 이 13개 안에 있다는 것은 07챕터·[03_MarketRelevance_판단.md](03_MarketRelevance_판단.md)의 "대기업 제외" 판단이 AOS·DOS 양쪽에서 모두 재확인된다는 뜻이다.

## 참고자료

- [00_방법론.md](00_방법론.md) 5절 — AOS·DOS 판단 기준 원문
- [02_AOS매트릭스.md](02_AOS매트릭스.md) — 36개 Pain의 AOS 원점수
- [04_DOS_SAM-SOM버전.md](04_DOS_SAM-SOM버전.md) — 36개 Pain의 DOS(SOM·SAM 버전) 원점수
- [이미지/AOS_DOS_비교.svg](../이미지/AOS_DOS_비교.svg) — AOS×DOS 산점도
