...
> ⚠️ **유의점 — BEST CASE 공동 1위(E1-②·E2-②)는 미검증 가설에 근거한다.** 두 항목의 핵심 근거인 "동시 마감 시 컷오프가 밀려 실제로 위약금을 냈다"는 사건은 05챕터 01문서 4단계가 이미 "원자료에 직접 인용이 없는 가설"로 표시해 둔 것이다. 은행 컷오프 시간·분기말 유동성 압박·지연배상금 조항이라는 개별 메커니즘 자체는 업계에서 잘 문서화된 실재하는 관행이지만(`01_Pain리스트_Goal추론.md` "외부 근거 보충" 절 참고), 이 셋이 실제로 결합해 "SME 수출입기업이 위약금을 낸" 사례를 뒷받침하는 업계 통계·사례는 확인되지 않았다. 즉 06챕터 전체(AOS→DOS→AOS×DOS)에서 일관되게 1위로 나온 항목이 **아직 실사되지 않은 추론**이라는 뜻이며, 07챕터 JTBD 인터뷰에서 발생 빈도·규모를 최우선으로 검증해야 한다.
...
같은 방식을 `04_DOS_SAM-SOM버전.md` 3절의 DOS(SAM 버전) 값에 그대로 적용했다. AOS는 Market Relevance와 무관하게 Importance·Satisfaction만으로 정해지므로 X축 좌표는 SOM 버전과 완전히 동일하다 — 즉 두 차트는 같은 AOS 열에서 Y축(DOS)만 다르게 움직이므로 나란히 비교하기에 알맞다. Y축 정규화도 SOM 버전과 같은 방식(0을 0.5로 고정하고, 최댓값 0.92를 0.99에, 최솟값 -1.16을 0.0에 선형으로 맞춤)을 썼다.
